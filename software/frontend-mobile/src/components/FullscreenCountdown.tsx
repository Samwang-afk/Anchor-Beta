...
export function FullscreenCountdown({ taskContent, onDone, title, subtitle, compact = false }: Props) {
  const [step, setStep] = useState(0)
  const sequence = compact ? SEQUENCE.slice(2) : SEQUENCE
  ...
  return (
    <div style={{
      ...
      {title && (
        <div style={{ textAlign: 'center', color: current.color }}>
          <p style={{ fontSize: 16, fontWeight: 800, letterSpacing: 0, textTransform: 'uppercase' }}>
            {title}
          </p>
          {subtitle && (
            <p style={{ marginTop: 8, fontSize: 28, fontWeight: 800, color: '#1a1a1a' }}>
              {subtitle}
            </p>
          )}
        </div>
