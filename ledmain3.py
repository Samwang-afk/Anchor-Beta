...
LED_UPDATE_INTERVAL_MS = 80
last_led_update_time = 0

COLOR_OFF = (0, 0, 0)

COLOR_MINT = (0, 180, 120)
COLOR_CYAN = (0, 120, 180)
COLOR_BLUE = (0, 50, 180)
COLOR_AMBER = (255, 120, 0)
COLOR_GREEN = (0, 200, 70)
COLOR_PURPLE = (120, 0, 180)
COLOR_WHITE = (180, 180, 180)

COLOR_PASTEL_BLUE = (120, 220, 255)
COLOR_PASTEL_MINT = (150, 255, 210)
COLOR_PASTEL_PURPLE = (230, 200, 255)
COLOR_PASTEL_PINK = (255, 190, 220)
...
def pastel_progress_color(index, total, brightness):
    if total <= 1:
        return scale_color(COLOR_PASTEL_MINT, brightness)

    t = index * 255 // (total - 1)

    if t < 85:
        color = blend_color(COLOR_PASTEL_BLUE, COLOR_PASTEL_MINT, t * 3)
    elif t < 170:
        color = blend_color(COLOR_PASTEL_MINT, COLOR_PASTEL_PURPLE, (t - 85) * 3)
    else:
        color = blend_color(COLOR_PASTEL_PURPLE, COLOR_PASTEL_PINK, (t - 170) * 3)

    return scale_color(color, brightness)


def led_set_all(color, brightness=80):
...
def led_undocked(now):
    ...
    c = scale_color(COLOR_PASTEL_BLUE, brightness)
...
def led_starting(now):
    ...
    for i in range(LED_COUNT):
        if i < progress:
            np[i] = pastel_progress_color(i, LED_COUNT, 110)
        else:
            np[i] = scale_color(COLOR_PASTEL_BLUE, 14)

    np.write()


def led_focusing(now):
    ...
    for i in range(LED_COUNT):
        if i < progress:
            np[i] = pastel_progress_color(i, LED_COUNT, 115)
        else:
            np[i] = scale_color(COLOR_PASTEL_BLUE, 12)

    if progress < LED_COUNT:
        pulse = (now // 120) % 6

        if pulse <= 2:
            highlight_brightness = 155
        else:
            highlight_brightness = 105

        np[progress] = pastel_progress_color(progress, LED_COUNT, highlight_brightness)

    np.write()


def led_break(now):
    pos = (now // 120) % LED_COUNT

    for i in range(LED_COUNT):
        np[i] = scale_color(COLOR_BLUE, 12)

    for offset in range(5):
        index = (pos + offset) % LED_COUNT
        brightness = 120 - offset * 18

        if brightness < 22:
            brightness = 22

        np[index] = scale_color(COLOR_PASTEL_BLUE, brightness)

    np.write()


def led_sync_lost(now):
    ...
    for i in range(LED_COUNT):
        if i < progress:
            np[i] = scale_color(COLOR_AMBER, 125)
        else:
            np[i] = scale_color(COLOR_AMBER, 14)

    np.write()


def led_return_buffer(now):
    ...
    for i in range(LED_COUNT):
        if i < lit:
            np[i] = scale_color(COLOR_AMBER, 70)
        else:
            np[i] = scale_color(COLOR_AMBER, 10)

    if phase == 0:
        np[0] = scale_color(COLOR_AMBER, 140)
        np[LED_COUNT - 1] = scale_color(COLOR_AMBER, 140)
...
def led_done(now):
    ...
    for i in range(LED_COUNT):
        color = wheel(shift + i * 8)
        np[i] = scale_color(color, 110)
